...
class _ServicesTabState extends ConsumerState<ServicesTab> {
  ...
  Future<void> _delete(CatalogService s) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('¿Desactivar "${s.name}"? Dejará de aparecer en el catálogo.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancelar')),
          TextButton(
            onPressed: () => Navigator.pop(context, true),
            child: const Text('Desactivar', style: TextStyle(color: AppColors.danger)),
          ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    final canWrite = perms.contains(Permissions.servicesWrite) || perms.contains('services.*');
                      ...
                      : RefreshIndicator(
                          ...
                          child: ListView.separated(
                            ...
                            itemBuilder: (_, i) => _ServiceCard(
                              ...
                              canWrite: canWrite,
                              onEdit: () async {
                                await context.push('/catalog/services/${_services[i].id}/edit', extra: _services[i]);
                                _load();
                              },
...
class _ServiceCard extends StatelessWidget {
  ...
  const _ServiceCard({required this.service, required this.canWrite, required this.onEdit, required this.onDelete});
      ...
      child: Row(
        children: [
          Container(
            ...
            child: const Icon(Icons.design_services_outlined, color: AppColors.success, size: 22),
          ),
          ...
          if (canWrite)
            PopupMenuButton<String>(
              ...
              itemBuilder: (_) => [
                ...
                const PopupMenuItem(value: 'delete', child: Text('Desactivar', style: TextStyle(color: AppColors.danger))),
              ],
            ),
        ],
      ),
